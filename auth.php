<?php
session_start();
require_once "config.php"; // Menggunakan koneksi MySQLi
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'login') {
  ...
  try {
    ...
    $stmt = $conn->prepare("SELECT * FROM users WHERE username = ? AND status = 'approved' LIMIT 1");
    ...
    // Menangkap error dari MySQLi
    $login_error = "Terjadi kesalahan koneksi ke database.";
    // Pada development, Anda bisa mencetak error: error_log($e->getMessage());
  }
}
  ...
  if (!is_sso_email($email)) {
    ...
    try {
      ...
      if ($existing) {
        ...
        $role = "mahasiswa";
        $status = "approved"; // Status langsung di-approve untuk demo
        ...
        $insert = $conn->prepare("
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
        ");
        $insert->bind_param("sssssss", $nama, $nim, $email, $username, $hashed_password, $role, $status);
        $insert->execute();
...
if (!isset($_SESSION['user_id']) && isset($_COOKIE['username'])) {
  ...
  try {
    ...
    // Diam-diam gagal, tidak perlu menampilkan error
