...
class JurusanManager {
    ...
    public function getJurusan($search = '', $limit = 10, $offset = 0) {
        try {
            $query = "SELECT * FROM jurusan WHERE 1=1";
            $params = [];
            $types = "";
            
            if (!empty($search)) {
                $query .= " AND nama_jurusan LIKE ?";
                $searchParam = "%{$search}%";
                $params[] = $searchParam;
                $types .= "s";
            }
            
            $query .= " ORDER BY nama_jurusan ASC LIMIT ? OFFSET ?";
            $params[] = $limit;
            $params[] = $offset;
            $types .= "ii";
    ...
    public function getTotalJurusan($search = '') {
        try {
            $query = "SELECT COUNT(*) as total FROM jurusan WHERE 1=1";
            $params = [];
            $types = "";
    ...
    public function createJurusan($data) {
        try {
            ...
            $stmt = $this->conn->prepare($query);
            $stmt->bind_param("s", $data['nama_jurusan']);
            $result = $stmt->execute();
            $stmt->close();
            return $result;
        } catch (Exception $e) {
    ...
    public function updateJurusan($id, $data) {
        try {
            ...
            $stmt = $this->conn->prepare($query);
            $stmt->bind_param("si", $data['nama_jurusan'], $id);
            $result = $stmt->execute();
            $stmt->close();
            return $result;
        } catch (Exception $e) {
    ...
    public function deleteJurusan($id) {
        try {
            // Cek apakah jurusan memiliki prodi terkait
            $checkQuery = "SELECT COUNT(*) as total FROM prodi WHERE id_jurusan = ?";
            ...
            if ($checkRow['total'] > 0) {
                return false; // Tidak bisa dihapus karena ada prodi terkait
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        $jurusanManager = new JurusanManager($conn);
        
        if (isset($_POST['action'])) {
            switch ($_POST['action']) {
                case 'add':
                    if ($jurusanManager->createJurusan($_POST)) {
                ...
                case 'edit':
                    $id = $_POST['id_jurusan'];
                    if ($jurusanManager->updateJurusan($id, $_POST)) {
                        $_SESSION['message'] = 'Jurusan berhasil diperbarui';
                        $_SESSION['message_type'] = 'success';
                    } else {
                        $_SESSION['message'] = 'Gagal memperbarui jurusan';
                        $_SESSION['message_type'] = 'danger';
                    }
                    header('Location: jurusan.php');
                    exit();
                ...
                case 'delete':
                    $id = $_POST['id_jurusan'];
                    if ($jurusanManager->deleteJurusan($id)) {
                        $_SESSION['message'] = 'Jurusan berhasil dihapus';
                        $_SESSION['message_type'] = 'success';
                    } else {
                        $_SESSION['message'] = 'Gagal menghapus jurusan. Jurusan mungkin memiliki program studi terkait.';
                        $_SESSION['message_type'] = 'danger';
                    }
                    header('Location: jurusan.php');
                    exit();
...
try {
    $jurusanManager = new JurusanManager($conn);
    $jurusanList = $jurusanManager->getJurusan($search, $limit, $offset);
    $totalJurusan = $jurusanManager->getTotalJurusan($search);
    $totalPages = ceil($totalJurusan / $limit);
} catch (Exception $e) {
    error_log("Error initializing JurusanManager: " . $e->getMessage());
    $jurusanList = [];
    $totalJurusan = 0;
    $totalPages = 0;
}
...
<head>
  ...
  <title>Manajemen Jurusan - Sistem Informasi Perpustakaan</title>
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
      <div class="sidebar-menu">
        <div class="menu-section">
          ...
          <a href="jurusan.php" class="menu-item active">
    ...
    <main class="main-content">
      <div class="content-header">
        <div>
          ...
          <p class="page-subtitle">Kelola data jurusan di sistem</p>
        </div>
        <div>
          <button class="action-btn primary" data-bs-toggle="modal" data-bs-target="#addJurusanModal">
        ...
        <form method="GET" action="jurusan.php">
          <div class="filter-section">
            <div class="filter-group">
              ...
              <input type="text" name="search" class="filter-input" placeholder="Nama jurusan" value="<?= htmlspecialchars($search) ?>">
            </div>
            <div class="filter-actions">
              <button type="submit" class="action-btn">
                <i class="bi bi-search me-2"></i>Cari
              </button>
              <a href="jurusan.php" class="action-btn">
                  ...
                  <tr>
                    ...
                    <td>
                      <div class="table-actions">
                        <button class="btn-action edit" data-bs-toggle="modal" data-bs-target="#editJurusanModal" 
                                data-id="<?= $jurusan['id_jurusan'] ?>" 
                                data-nama="<?= htmlspecialchars($jurusan['nama_jurusan']) ?>"
                                title="Edit">
                          <i class="bi bi-pencil"></i>
                        </button>
                        <button class="btn-action delete" data-bs-toggle="modal" data-bs-target="#deleteJurusanModal" 
                                data-id="<?= $jurusan['id_jurusan'] ?>" 
                                data-nama="<?= htmlspecialchars($jurusan['nama_jurusan']) ?>"
                                title="Hapus">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="addJurusanModalLabel">Tambah Jurusan Baru</h5>
  ...
  <div class="modal fade" id="deleteJurusanModal" tabindex="-1" aria-labelledby="deleteJurusanModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="deleteJurusanModalLabel">Konfirmasi Hapus</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <form method="POST" action="jurusan.php">
          <div class="modal-body">
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (editJurusanModal) {
        editJurusanModal.addEventListener('show.bs.modal', function(event) {
      ...
      // Delete Jurusan Modal
      const deleteJurusanModal = document.getElementById('deleteJurusanModal');
      if (deleteJurusanModal) {
        deleteJurusanModal.addEventListener('show.bs.modal', function(event) {
          ...
          document.getElementById('deleteJurusanNama').textContent = nama;
        });
      }
    });
  </script>
