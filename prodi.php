...
class ProdiManager {
    ...
    public function getProdi($search = '', $limit = 10, $offset = 0) {
        try {
            // Query dengan JOIN untuk mendapatkan nama jurusan
            ...
            if (!empty($search)) {
                $query .= " AND (p.nama_prodi LIKE ? OR j.nama_jurusan LIKE ?)";
                $searchParam = "%{$search}%";
                $params[] = $searchParam;
                $params[] = $searchParam;
                $types .= "ss";
            ...
            $query .= " ORDER BY j.nama_jurusan ASC, p.nama_prodi ASC LIMIT ? OFFSET ?";
            $params[] = $limit;
            $params[] = $offset;
            $types .= "ii";
    ...
    public function getTotalProdi($search = '') {
        try {
    ...
    public function getProdiById($id) {
        try {
            ...
            error_log("Error getProdiById: " . $e->getMessage());
            return null;
        }
    }
    ...
    public function createProdi($data) {
        try {
            $query = "INSERT INTO prodi (nama_prodi, id_jurusan) VALUES (?, ?)";
            $stmt = $this->conn->prepare($query);
            $stmt->bind_param("si", $data['nama_prodi'], $data['id_jurusan']);
            $result = $stmt->execute();
            $stmt->close();
            return $result;
        } catch (Exception $e) {
            error_log("Error createProdi: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function updateProdi($id, $data) {
        try {
            $query = "UPDATE prodi SET nama_prodi = ?, id_jurusan = ? WHERE id_prodi = ?";
            $stmt = $this->conn->prepare($query);
            $stmt->bind_param("sii", $data['nama_prodi'], $data['id_jurusan'], $id);
            $result = $stmt->execute();
            $stmt->close();
            return $result;
        } catch (Exception $e) {
            error_log("Error updateProdi: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function deleteProdi($id) {
        try {
            // Cek apakah prodi digunakan di tabel lain
            $checkQuery = "SELECT COUNT(*) as total FROM distribusi_dokumen WHERE id_prodi = ?";
            ...
            if ($checkRow['total'] > 0) {
                return false; // Tidak bisa dihapus karena ada dokumen terkait
            }
            ...
            error_log("Error deleteProdi: " . $e->getMessage());
            return false;
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        $prodiManager = new ProdiManager($conn);
        
        if (isset($_POST['action'])) {
            switch ($_POST['action']) {
                case 'add':
                    if ($prodiManager->createProdi($_POST)) {
                        $_SESSION['message'] = 'Program Studi berhasil ditambahkan';
                        $_SESSION['message_type'] = 'success';
                    } else {
                        $_SESSION['message'] = 'Gagal menambahkan Program Studi';
                ...
                case 'edit':
                    $id = $_POST['id_prodi'];
                    if ($prodiManager->updateProdi($id, $_POST)) {
                        $_SESSION['message'] = 'Program Studi berhasil diperbarui';
                        $_SESSION['message_type'] = 'success';
                    } else {
                        $_SESSION['message'] = 'Gagal memperbarui Program Studi';
                        $_SESSION['message_type'] = 'danger';
                    }
                    header('Location: prodi.php');
                    exit();
                ...
                case 'delete':
                    $id = $_POST['id_prodi'];
                    if ($prodiManager->deleteProdi($id)) {
                        $_SESSION['message'] = 'Program Studi berhasil dihapus';
                        $_SESSION['message_type'] = 'success';
                    } else {
                        $_SESSION['message'] = 'Gagal menghapus Program Studi. Program Studi mungkin memiliki dokumen terkait.';
                        $_SESSION['message_type'] = 'danger';
                    }
                    header('Location: prodi.php');
                    exit();
...
try {
    $prodiManager = new ProdiManager($conn);
    $prodiList = $prodiManager->getProdi($search, $limit, $offset);
    $totalProdi = $prodiManager->getTotalProdi($search);
    $totalPages = ceil($totalProdi / $limit);
    $jurusanList = $prodiManager->getJurusan();
} catch (Exception $e) {
    error_log("Error initializing ProdiManager: " . $e->getMessage());
    $prodiList = [];
    $totalProdi = 0;
    $totalPages = 0;
    $jurusanList = [];
...
<head>
  ...
  <title>Manajemen Program Studi - Sistem Informasi Perpustakaan</title>
...
<body>
  ...
  <div class="main-container">
    <aside class="sidebar" id="sidebar">
      <div class="sidebar-menu">
        <div class="menu-section">
          ...
          <a href="prodi.php" class="menu-item active">
            <i class="bi bi-building"></i>
            Prodi
          </a>
        </div>
    ...
    <main class="main-content">
      <div class="content-header">
        <div>
          <h1 class="page-title">Manajemen Program Studi</h1>
          <p class="page-subtitle">Kelola data program studi di setiap jurusan</p>
        </div>
        <div>
          <button class="action-btn primary" data-bs-toggle="modal" data-bs-target="#addProdiModal">
        ...
        <form method="GET" action="prodi.php">
          <div class="filter-section">
            <div class="filter-group">
              ...
              <input type="text" name="search" class="filter-input" placeholder="Nama prodi atau jurusan" value="<?= htmlspecialchars($search) ?>">
            </div>
            <div class="filter-actions">
              <button type="submit" class="action-btn">
                <i class="bi bi-search me-2"></i>Cari
              </button>
              <a href="prodi.php" class="action-btn">
            ...
            <tbody>
              <?php if (!empty($prodiList)): ?>
                ...
                <?php foreach ($prodiList as $prodi): ?>
                  <tr>
                    <td><?= $no++ ?></td>
                    <td><?= htmlspecialchars($prodi['id_prodi']) ?></td>
                    <td><?= htmlspecialchars($prodi['nama_prodi']) ?></td>
                    <td><?= htmlspecialchars($prodi['nama_jurusan']) ?></td>
                    <td>
                      <div class="table-actions">
                        <button class="btn-action edit" data-bs-toggle="modal" data-bs-target="#editProdiModal" 
                                data-id="<?= $prodi['id_prodi'] ?>" 
                                data-nama="<?= htmlspecialchars($prodi['nama_prodi']) ?>"
                                data-jurusan="<?= $prodi['id_jurusan'] ?>"
                                title="Edit">
                          <i class="bi bi-pencil"></i>
                        </button>
                        <button class="btn-action delete" data-bs-toggle="modal" data-bs-target="#deleteProdiModal" 
                                data-id="<?= $prodi['id_prodi'] ?>" 
                                data-nama="<?= htmlspecialchars($prodi['nama_prodi']) ?>"
                                title="Hapus">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="addProdiModalLabel">Tambah Program Studi Baru</h5>
      ...
      <div class="modal-content">
        ...
        <form method="POST" action="prodi.php">
          <div class="modal-body">
            ...
            <input type="hidden" name="id_prodi" id="editProdiId">
            <div class="form-group">
              <label class="form-label">Nama Program Studi</label>
              <input type="text" name="nama_prodi" id="editProdiNama" class="form-control" required>
            </div>
            <div class="form-group">
              <label class="form-label">Jurusan</label>
              <select name="id_jurusan" id="editProdiJurusan" class="form-control form-select" required>
                <option value="">Pilih Jurusan</option>
                <?php if (!empty($jurusanList)): ?>
                  <?php foreach ($jurusanList as $jurusan): ?>
                    <option value="<?= $jurusan['id_jurusan'] ?>"><?= htmlspecialchars($jurusan['nama_jurusan']) ?></option>
      ...
      <div class="modal-content">
        ...
        <form method="POST" action="prodi.php">
          <div class="modal-body">
            ...
            <input type="hidden" name="id_prodi" id="deleteProdiId">
            <p>Apakah Anda yakin ingin menghapus program studi <strong id="deleteProdiNama"></strong>?</p>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (editProdiModal) {
        editProdiModal.addEventListener('show.bs.modal', function(event) {
          ...
          document.getElementById('editProdiJurusan').value = jurusan;
        });
      }
      ...
      if (deleteProdiModal) {
        deleteProdiModal.addEventListener('show.bs.modal', function(event) {
          ...
          document.getElementById('deleteProdiNama').textContent = nama;
        });
      }
    });
  </script>
